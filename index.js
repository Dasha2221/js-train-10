...
function customEntries(obj) {
  ...
  const objEntris = Object.entries(obj);
  return objEntris;
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const getObject = Object.assign(obj1, obj2);

  Object.freeze(getObject);
  if (Object.isFrozen(getObject)) {
    return getObject;
  } else {
...
function keysAndValues(obj) {
  ...
  const keysAnd = Object.keys(obj);
  const valuesAnd = Object.values(obj);
  return [keysAnd, valuesAnd];
...
function getObjectValuesSum(obj) {
  ...
  const getObject = Object.values(obj);
  const valuesSum = getObject.reduce(
...
function convertArrayToObj(arr) {
  ...
  for (const [key, value] of arr) {
    const count = arr.filter((item) => item[0] === key).length;
    if (count > obj) {
